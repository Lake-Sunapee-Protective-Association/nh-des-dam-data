...
dam_2021 <- full_join(airtemp_out_flat, elev_out_flat) %>% 
  full_join(., flow_out_flat) %>% 
  full_join(., precip_out_flat)

#convert each column to character from a list
dam_2021 <- dam_2021 %>% 
  mutate_at(vars(all_of(colnames(dam_2021))),
            ~as.character(.))

#give new column names
names(dam_2021) <- c('data_status', 'location', 'parameter', 'date', 'value')
...
write.csv(dam_2021, 'data/sunapee_NHDESDB_2021.csv', row.names = F)
