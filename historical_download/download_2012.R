...
dam_2012 <- elev_out_flat

#convert each column to character from a list
dam_2012 <- dam_2012 %>% 
  mutate_at(vars(all_of(colnames(dam_2012))),
            ~as.character(.))

#give new column names
names(dam_2012) <- c('data_status', 'location', 'parameter', 'date', 'value')
...
write.csv(dam_2012, 'data/sunapee_NHDESDB_2012.csv', row.names = F)
