...
dam_2013 <- elev_out_flat

#convert each column to character from a list
dam_2013 <- dam_2013 %>% 
  mutate_at(vars(all_of(colnames(dam_2013))),
            ~as.character(.))

#give new column names
names(dam_2013) <- c('data_status', 'location', 'parameter', 'date', 'value')
...
write.csv(dam_2013, 'data/sunapee_NHDESDB_2013.csv', row.names = F)
