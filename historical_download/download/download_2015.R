...
dam_2015 <- elev_out_flat

#convert each column to character from a list
dam_2015 <- dam_2015 %>% 
  mutate_at(vars(all_of(colnames(dam_2015))),
            ~as.character(.))

#give new column names
names(dam_2015) <- c('data_status', 'location', 'parameter', 'date', 'value')
...
write.csv(dam_2015, 'data/sunapee_NHDESDB_2015.csv', row.names = F)
