...
dam_2020 <- full_join(airtemp_out_flat, elev_out_flat) %>% 
  full_join(., flow_out_flat) %>% 
  full_join(., precip_out_flat)

#convert each column to character from a list
dam_2020 <- dam_2020 %>% 
  mutate_at(vars(all_of(colnames(dam_2020))),
            ~as.character(.))

#give new column names
names(dam_2020) <- c('data_status', 'location', 'parameter', 'date', 'value')
...
write.csv(dam_2020, 'data/sunapee_NHDESDB_2020.csv', row.names = F)
