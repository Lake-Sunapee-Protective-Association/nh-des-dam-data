...
dam_2014 <- elev_out_flat

#convert each column to character from a list
dam_2014 <- dam_2014 %>% 
  mutate_at(vars(all_of(colnames(dam_2014))),
            ~as.character(.))

#give new column names
names(dam_2014) <- c('data_status', 'location', 'parameter', 'date', 'value')
...
write.csv(dam_2014, 'data/sunapee_NHDESDB_2014.csv', row.names = F)
