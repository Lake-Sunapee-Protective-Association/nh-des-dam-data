...
dam_e2022 <- full_join(airtemp_out_flat, elev_out_flat) %>% 
  full_join(., flow_out_flat) %>% 
  full_join(., precip_out_flat)

#convert each column to character from a list
dam_e2022 <- dam_e2022 %>% 
  mutate_at(vars(all_of(colnames(dam_e2022))),
            ~as.character(.))

#give new column names
names(dam_e2022) <- c('data_status', 'location', 'parameter', 'date', 'value')
...
write.csv(dam_e2022, 'data/sunapee_NHDESDB_e2022.csv', row.names = F)
