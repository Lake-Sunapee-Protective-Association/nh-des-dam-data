...
dam_2011 <- elev_out_flat

#convert each column to character from a list
dam_2011 <- dam_2011 %>% 
  mutate_at(vars(all_of(colnames(dam_2011))),
            ~as.character(.))

#give new column names
names(dam_2011) <- c('data_status', 'location', 'parameter', 'date', 'value')
...
write.csv(dam_2011, 'data/sunapee_NHDESDB_2011.csv', row.names = F)
